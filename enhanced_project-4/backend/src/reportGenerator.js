...
export async function generateReport(metrics, options = {}) {
  ...
  let logoBuffer;
  try {
    logoBuffer = fs.readFileSync(path.join(__dirname, '../../frontend/logo.png'));
  } catch {
  ...
  // Adjust the logo width to make it more prominent in the header.
  // Increasing this value scales the logo proportionally while preserving the
  // aspect ratio. A width of 140 provides a larger, yet balanced, appearance
  // on the A4 page without overwhelming the title or date.
